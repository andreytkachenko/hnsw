...
use crate::{
    heap::{DistanceCache as _, DistanceOrderedHeap, Heap, Wrap},
    index::HnswConfig,
...
impl<S: Scalar> HnswLayer<S> {
    ...
    #[inline]
    pub fn create_node(&self, vector: &[S], entrypoint: u32) -> u32 {
        ...
        if self.nodes.count() > 1 {
            ...
            self.search(vector, entrypoint, &mut heap, false);
...
mod test {
    ...
    fn test_layer() {
        ...
        for i in 0..32 {
            for j in 0..32 {
                layer.create_node(&[i as f32, j as f32], 0);
            }
        }

        let mut res: ArrayVec<u32, 4> = ArrayVec::new();
        let mut heap = DistanceOrderedHeap::new(&mut res, Some(4), None);

        let best_one = layer.search(&[31.5f32, 31.5f32], 0, &mut heap, true);
